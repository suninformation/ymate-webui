...
<div <c:if test="${not empty _id}">id="${_id}"
     </c:if>class="tabbable<c:choose><c:when test="${line}">-line</c:when><c:when test="${custom}">-custom<c:if test="${justified}"> nav-justified</c:if></c:when></c:choose><c:if test="${below}"> tabs-below</c:if><c:if test="${not empty _class}"><%=" "%>${_class}</c:if>"
