...
<%-- Tag Body --%>
<li <c:if test="${not empty _id}">id="${_id}" </c:if>class="dd-item<c:if test="${not empty _class}"><%=" "%>${_class}</c:if>"<c:if test="${not empty _style}"> style="${_style}"</c:if> data-id="${dataId}"<c:if test="${not empty _attrs}"><%=" "%>${_attrs}</c:if>>
