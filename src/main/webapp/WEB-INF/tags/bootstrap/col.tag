...
<%-- Tag Body --%>
<div <c:if test="${not empty _id}">id="${_id}" </c:if>class="<c:if test="${md > 0}">col-md-${md} <c:if test="${mdOffset > 0}"> col-md-offset-${mdOffset}</c:if><c:if test="${mdPush > 0}"> col-md-push-${mdPush}</c:if><c:if test="${mdPull > 0}"> col-md-pull-${mdPull}</c:if></c:if>
