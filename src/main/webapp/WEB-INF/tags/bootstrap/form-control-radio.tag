...
<%-- Tag Body --%>
<c:if test="${!inline}"><div class="radio<c:if test="${disabled}"> disabled</c:if>"></c:if>
    <label<c:if test="${inline}"> class="radio-inline<c:if test="${disabled}"> disabled</c:if>"</c:if>>
        <input
        <c:if test="${not empty id}"> id="${id}"</c:if> type="radio"
        <c:if test="${not empty name}"> name="${name}"</c:if> value="${value}" <c:if test="${disabled}"> disabled</c:if><c:if test="${checked}"> checked</c:if>>
        <jsp:doBody/>
    </label>
