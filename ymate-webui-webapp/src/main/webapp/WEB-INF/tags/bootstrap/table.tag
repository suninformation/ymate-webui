...
<table <c:if test="${not empty _id}">id="${_id}" </c:if>class="table<c:if test="${striped}"> table-striped</c:if><c:if test="${bordered}"> table-bordered</c:if><c:if test="${hover}"> table-hover</c:if><c:if test="${condensed}"> table-condensed</c:if>
